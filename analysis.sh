...
if [ -f /etc/redhat-release ]; then
    OS_LIKE=$(cat /etc/redhat-release | awk '{print $1}')
fi
...
users_list() {
    ...
    echo "Пользователи с /home/:" 
    {
        ...
        if [ -d `/lost+found`]; then
            users=$(ls /home -I lost*) 2>/dev/null
        else
            users=$(ls /home)
        fi
        echo $users 2>/dev/null
        echo -e "\n" 
    ...
    find /root /home /bin /etc /lib64 /opt /run /usr -type f -name \*log* >> change_files 
